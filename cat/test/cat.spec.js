...
cat.prefix(path.join(__dirname, "../.."));
...
describe("Catalog", function() {
  it("module contains classes with qualified names", function() {
    ...
    expect(A[CAT].qname).to.equal("./cat/test/cat.spec/A");
  })
  ...
  it("module contains functions with qualified names", function() {
    ...
    expect(add[CAT].qname).to.equal("./cat/test/cat.spec/add");
  })
  ...
  it("modules can be located by name", function() {
    expect(cat("./cat/test/cat.spec").target).to.equal(module.exports);
  })

  it("classes can be located by name", function() {
    expect(cat("./cat/test/cat.spec/A").target).to.equal(A);
  })

  it("functions can be located by name", function() {
    let fn = cat("./cat/test/cat.spec/add").target;
    expect(fn).to.equal(add);
    expect(fn(1, 2)).to.equal(3);
  })

  it("can configure aliases for module", function() {
    expect(cat("./cat/test/cat.spec/alt/location/A").target).to.equal(A);
    expect(cat("./cat/test/cat.spec/alt/location/add").target).to.equal(add);
  })
  ...
  it("has qualified name", function() {
    ...
    expect(c.qname).to.equal("./cat/test/cat.spec/a/b/c");
  })

  it("at top level qualified name", function() {
    let c = cat(__filename, "a");
    expect(c.qname).to.equal("./cat/test/cat.spec/a");
  })
